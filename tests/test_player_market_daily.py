...
def test_player_market_daily_transform(spark, generate_mock_data, generate_mock_data_md):
    # Use mock data fixture for both bets and market datasets
    mock_bets_data = generate_mock_data(data_points=1000)
    mock_market_data = generate_mock_data_md()
    ...
    pmd = PlayerMarketDailyTransform(mock_bets_dataset, mock_market_dataset)
    result = pmd.transform()
